...
def view_members(family_id):
    ...
    if search_query:
            ...
            cur.execute("SELECT patients_record.*, consultation_record.systolic FROM patients_record JOIN consultation_record ON patients_record.patients_id = consultation_record.patients_id WHERE patients_record.family_id = %s AND patients_record.firstname LIKE %s LIMIT %s OFFSET %s", (family_id, f"%{search_query}%", per_page, (page - 1) * per_page))

    else:
            cur.execute("SELECT COUNT(*) FROM patients_record WHERE family_id = %s", (family_id,))
            total_records = cur.fetchone()[0]

            cur.execute("SELECT patients_record.*, consultation_record.systolic FROM patients_record JOIN consultation_record ON patients_record.patients_id = consultation_record.patients_id WHERE patients_record.family_id = %s AND patients_record.firstname LIKE %s LIMIT %s OFFSET %s", (family_id, '%', per_page, (page - 1) * per_page))
...
def Prediction():
    ...
    if search_query:
        ...
        cur.execute("SELECT prr.patients_id, prr.prediction_number, pr.lastname, pr.firstname, pr.middlename, prr.sex, prr.age, prr.total_chol, prr.hdl_chol, prr.sbp, prr.smoker, prr.treated_bp, prr.risk_score FROM patients_record pr INNER JOIN prediction_record prr ON pr.patients_id = prr.patients_id WHERE prr.patients_id LIKE %s OR pr.firstname LIKE %s OR pr.lastname LIKE %s LIMIT %s OFFSET %s", (f"%{search_query}%", f"%{search_query}%", f"%{search_query}%", per_page, (page - 1) * per_page))
    else:
        cur.execute("SELECT COUNT(*) FROM prediction_record")
        total_records = cur.fetchone()[0]

        cur.execute(" SELECT prr.patients_id, prr.prediction_number, pr.lastname, pr.firstname, pr.middlename, prr.sex, prr.age, prr.total_chol, prr.hdl_chol, prr.sbp,prr.smoker, prr.treated_bp, prr.risk_score FROM patients_record pr INNER JOIN prediction_record prr ON pr.patients_id = prr.patients_id LIMIT %s OFFSET %s", (per_page, (page - 1) * per_page))
...
def insert_prediction():
    if request.method == "POST":
        ...
        cur.execute("SELECT age,sex FROM patients_record  WHERE patients_id = %s", [patients_id])
        ...
        new_data = [age, sex, total_chol, hdl_chol, sbp, treated_bp, smoker]
        ...
        cur.execute("INSERT INTO prediction_record (patients_id, sex, age, smoker, total_chol, hdl_chol, sbp, treated_bp, risk_score) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)", (patients_id, sex, age, smoker, total_chol, hdl_chol, sbp, treated_bp, risk_score))
        mysql.connection.commit()
